#!/usr/bin/env python3
"""
Oracle SICA for FOLIO (exp-089) Mistral-7B: gold FOL + Z3 entailment + oracle-weighted voting.
Uses exp-033 Mistral-7B traces with gold FOL annotations.
...
def main():
    ...
    summary = {
        ...
        "traces_source": "exp033-mistral-7b-folio204",
    ...
    summary["comparison_exp021"] = {
        ...
        "exp089_oracle": round(oa, 4), "exp089_sc": round(sa, 4), "exp089_z3_agree": round(za, 4),
        "note": "exp-021: old traces pre-normalization-fix; exp-089: exp-033 Mistral-7B traces",
    }

    output = {"summary": summary, "results": results}
    out_path = os.path.join(output_dir, "exp089_results.json")
    with open(out_path, "w") as f:
        json.dump(output, f, indent=2, default=str)
    with open(os.path.join(output_dir, "exp089_summary.json"), "w") as f:
        json.dump(summary, f, indent=2)

    print("\n" + "="*70)
    print("Oracle SICA Results (exp-089-oracle-folio-mistral7b)")
    print("="*70)
    print(f"N = {n}, K = 12")
    ...
    print(f"  exp-089: oracle={oa:.4f}  sc={sa:.4f}  z3_agree={za:.3f}")
