#!/usr/bin/env python3
"""Qwen2.5-14B ProofWriter NLI Combo using D116's DeBERTa-large predictions.
...
def main():
    ...
    with open(EXP032_PATH) as f:
        exp032 = json.load(f)
    with open(D116_PATH) as f:
        d116 = json.load(f)

    results_list = exp032["results"]
    d116_pq = d116["per_question"]
    total = len(results_list)
    print(f"Loaded {total} problems from exp032 (Qwen2.5-14B PW600)")

    # Extract per-problem data from exp032
    gt = {}
    sc_answers = {}
    sc_vote_dists = {}
    ...
    # Verify all problem_ids match D116
    ...
    # Verify D116 gold labels match exp032
    ...
    print(f"\nSC baseline (Qwen2.5-14B, K=12): {sc_acc:.4f} ({sc_correct_n}/{total})")

    # DeBERTa standalone (reused from D116)
    ...
    print(f"\nComplementarity: NLI✓SC✗={nli_right_sc_wrong}, SC✓NLI✗={sc_right_nli_wrong}, both✓={both_right}, both✗={both_wrong}")
    ...
    for w in WEIGHTS:
        combo_correct = 0
        b_count = 0  # combo right, SC wrong
        c_count = 0  # combo wrong, SC right

        for pid in gt:
            gold = gt[pid]
            nli_pred = deberta_preds[pid]
            sc_dist = sc_vote_dists[pid]

            combo_votes = {}
            for cls in CLASSES:
                combo_votes[cls] = sc_dist.get(cls, 0) + (w if cls == nli_pred else 0)
            _max = max(combo_votes.values()); combo_answer = sorted(k for k, v in combo_votes.items() if v == _max)[0]
    ...
    # Save
    output = {
        "experiment": "Qwen2.5-14B ProofWriter NLI Combo (DeBERTa-large reused from D116)",
