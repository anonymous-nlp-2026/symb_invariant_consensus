...
\paragraph{System Prompt.}\leavevmode

\begin{lstlisting}
You are a constraint extraction tool. Output ONLY valid JSON. Do NOT solve problems, reason about correctness, or think step-by-step. Extract constraints from the given trace exactly as stated.
\end{lstlisting}
...
\begin{lstlisting}
Extract mathematical constraints from this reasoning trace as a JSON object.

Rules:
- type: "equation", "inequality", or "predicate"
- expression: human-readable, e.g. "x + y == 14"
- z3_formula: a single comparison expression in Python Z3 syntax
- source_step: step number where the constraint appears
- Use variable names from the trace

z3_formula MUST follow these rules:
- Allowed operators: ==, !=, <, >, <=, >=, +, -, *, /, **, %
- Use individual named variables: a1, a2, a3... NOT arrays a[1] or sets A
- Expand all sums explicitly: "a1 + a2 + a3 == 55" NOT "sum([a1,a2,a3])"
- NO Python builtins: sum(), len(), max(), min(), list(), set(), range(), int()
- Each z3_formula must be exactly ONE comparison (==, !=, <, >, <=, >=)
- For multiple values, write separate constraints for each
...
\begin{lstlisting}
IMPORTANT: You are an EXTRACTION tool, NOT a problem solver.
- Do NOT re-solve or re-reason about the problem.
- Do NOT verify whether the trace's reasoning is correct.
- ONLY extract the logical constraints that are explicitly stated or used in the trace below.
- Output ONLY the JSON object, nothing else.

Extract logical constraints from this reasoning trace about a logic problem.

The problem has facts (base assertions), rules (grounded implications), and derived conclusions. Convert each into a Z3 boolean formula.
...
\end{equation}
$\text{CCR} > 0$ means $V$ preferentially corrects $G$'s errors; $\text{CCR} < 0$ indicates shared bias. CCR links our findings: (1)~SCB corresponds to $\text{CCR}(\text{self}, G) \approx 0$; (2)~the cross-architecture escape (\S\ref{sec:cross-arch}) predicts $\text{CCR}(V, G) > 0$ when $V$ is independent and capable.
...
Table~\ref{tab:remediation} reports nine prompt-level strategies; Table~\ref{tab:method_interventions} reports three method-level interventions. Figure~\ref{fig:interventions-summary} visualizes all 27 strategies.

\begin{figure*}[t]
\centering
\includegraphics[width=0.82\textwidth]{figures/fig2_interventions_compact.pdf}
...
\centering
\small
\caption{Method-level interventions on Mistral-7B with McNemar's paired test. All produce neutral-to-catastrophic results. Independent Constraint Generation (ICG) shows two FOLIO seeds; $p$ for the first. {*}{*}{*}\,$p < 0.001$; {*}{*}\,$p < 0.01$.}
...
Table~\ref{tab:additional_interventions} reports the remaining 15 strategies beyond the 12 in Appendix~\ref{app:remediation}.
...
Figure~\ref{fig:process-kappa} (main text) visualizes the process--answer independence gap.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.75\textwidth]{f3_k_sensitivity}
...
\centering
\small
\caption{Entropy decomposition of SICA $\Delta$ (pp). High-entropy: majority count $\leq 8$; low-entropy: majority count $\geq 9$. $^\dagger$Partial data (180/600). Pre-tiebreaking SC values; pattern robust to BH correction.}
\label{tab:entropy}
...
Table~\ref{tab:cross-model-bias} reports MaxSAT-weighted confirmation bias metrics across three model families on FOLIO ($n = 204$). Weighted bias ratio and wrong-supporting constraint percentage both decrease with model capability, while SICA's accuracy effect shifts from positive (Mistral, intermediate-SC regime) to negative (LLaMA, high-SC regime). Raw bias ratios ($4.97\times$, $3.93\times$, $3.87\times$) are reported in \S\ref{sec:confirmation-bias}. LLaMA raw BR CI: [3.11, 4.98] (4/12 traces).
...
Table~\ref{tab:frontier-scb} (main text) reports confirmation bias across nine models spanning 7B open-source to frontier closed-source. All models exhibit $\text{BR} > 1$ ($1.66$--$4.97\times$). Open-source models use self-extraction; frontier models use cross-model extraction (Mistral-7B extracts FOL from target traces), as closed-source APIs do not expose internal representations suitable for direct symbolic extraction. BR estimates for high-accuracy models ($n_\text{err} < 40$) have wider confidence intervals; Gemini ($n_\text{err} \approx 12$) should be interpreted with particular caution. Process-level similarity ($\kappa_{\text{proc}}$) ranges from $0.25$ (R1-Distill) to $0.79$ (gpt-4o/4.1/Gemini); see Table~\ref{tab:cross-model-process}.
...
The independence-accuracy scatter plot is presented in Figure~\ref{fig:kappa-accuracy} in the main text.
...
Observation~1 assumes approximately uniform constraint yield per trace within each answer group. We verify this on Mistral-7B FOLIO ($K{=}12$, $n{=}204$). Per-problem yield CV (coefficient of variation across traces within each problem) has median $0.257$ and mean $0.319$; 64.2\% of problems fall below $\text{CV} = 0.3$, and 91.2\% below $0.5$, supporting the uniformity assumption (Figure~\ref{fig:cv-distribution}). Majority-answer traces average $5.61$ constraints and minority-answer traces $6.04$ (Mann--Whitney $p = 5.2 \times 10^{-6}$); correct-answer traces average $5.41$ and incorrect $6.05$ ($p = 5.3 \times 10^{-11}$). Although statistically significant, the absolute difference (${\sim}0.6$ constraints, $< 12\%$) is insufficient to alter the argmax-preservation result: SICA and SC select the same answer on ${>}96\%$ of problems regardless of yield variation. On the 18 problems with CV${>}0.5$, SICA and SC select the same answer on 18/18 (100\%), confirming that yield non-uniformity does not alter the degeneration pattern.

\begin{figure}[h]
\centering
\includegraphics[width=0.8\columnwidth]{figures/fig_cv_distribution.pdf}
\caption{Distribution of per-problem within-group CV for constraint yield (Mistral-7B, FOLIO, $n{=}204$). Median CV${=}0.26$ (red dashed); 64\% of problems have CV${\leq}0.3$. Five problems with CV${>}1.0$ (max $3.46$) are clipped for readability.}
...
\label{app:drc}
Disagreement-Routed Cascade (DRC) invokes NLI only when SC consensus falls below threshold $\tau$, avoiding unnecessary verifier calls on high-agreement problems. Gains concentrate on weak generators (SC $< 40\%$), consistent with the capability-gap hypothesis (\S\ref{sec:cross-arch}). Regime-dependent bottleneck analysis appears in Appendix~\ref{app:2x2}.
...
Figure~\ref{fig:capability-gap} (main text) visualizes the capability-gap pattern. Table~\ref{tab:cross_generator_nli} reports the controlled cross-generator comparison with DeBERTa-large-mnli on ProofWriter ($n{=}600$). We extend this analysis by pairing Mistral-7B with two additional NLI architectures: BART-large-mnli (46.17\% standalone, combo $+5.17$\,pp, $p = 0.0002$) and RoBERTa-large-mnli (37.67\% standalone, combo $+4.17$\,pp, $p = 0.003$). All three NLI architectures produce significant gains on Mistral-7B at $w \geq 3$.
...
\centering
\small
\caption{Domain-adapted NLI combo on ProofWriter depth-5 ($n{=}600$). DeBERTa-large fine-tuned on ProofWriter Open-World Assumption (OWA) (99.67\% standalone). All $p < 0.0001$. Complementarity: ratio of NLI-correct$\cap$SC-incorrect to SC-correct$\cap$NLI-incorrect.}
...
\centering
\small
\caption{Multi-seed replication results across six model$\times$dataset conditions. Each row uses an independent seed with fresh traces ($K{=}12$, $T{=}0.7$). Mistral-7B$\times$FOLIO shows directionally unstable $\Delta$ ($+0.16 \pm 2.70$\,pp). Mistral-7B, LLaMA-8B, and Qwen2.5-14B show consistently negative $\Delta$ on ProofWriter ($p < .001$ for LLaMA-8B and Mistral-7B); Qwen3-14B shows negligible mixed-sign $\Delta$ ($+0.92 \pm 1.14$\,pp, all n.s.), consistent with its high SC baseline minimizing correctable errors (\S\ref{sec:discussion}).}
...
\midrule
\multicolumn{4}{@{}l}{\textit{Qwen3-14B $\times$ ProofWriter ($n{=}600$)}} \\
42  & 85.83 & 85.67 & $-$0.17 \\
123 & 85.17 & 86.50 & $+$1.33 \\
456 & 84.50 & 86.83 & $+$2.33 \\
789 & 85.50 & 85.67 & $+$0.17 \\
\midrule
Mean $\pm$ SD & 85.25 $\pm$ 0.57 & 86.17 $\pm$ 0.59 & $+$0.92 $\pm$ 1.14 \\
\bottomrule
\end{tabular}}
\vspace{2pt}
{\footnotesize \textsuperscript{a}Original baseline used no fixed seed and is excluded from seed statistics.}
\end{table}

On ProofWriter, all twelve Mistral-7B, LLaMA-8B, and Qwen2.5-14B seeds yield negative $\Delta$ (range $-0.83$ to $-7.50$\,pp), with all LLaMA-8B McNemar tests reaching $p < .001$. The results reveal a capability-dependent gradient: LLaMA-8B $\Delta{=}-6.25$ $\to$ Mistral-7B $\Delta{=}-2.29$ $\to$ Qwen2.5-14B $\Delta{=}-1.71$ $\to$ Qwen3-14B $\Delta{=}+0.92$\,pp. Qwen3-14B is the sole exception: its 4-seed mean $\Delta$ is slightly positive ($+0.92 \pm 1.14$\,pp, all n.s.; 95\% CI includes zero), reflecting the capability-modulated pattern discussed in \S\ref{sec:discussion}: models with SC baselines above 85\% leave few correctable errors for SICA to exploit. The consistency across seeds confirms that the sign of $\Delta$ on ProofWriter is not an artifact of any single random sample but reflects model-level reasoning capacity. SC ties are broken alphabetically; across 2{,}400 ProofWriter problems ($4 \times 600$), zero SC--SICA answer differences were attributable to tiebreaking, confirming that ties are too rare at $\kappa \approx 0.78$ to affect $\Delta$.
...
\label{app:per-trace-separation}

\paragraph{Per-trace separation analysis.}
On Mistral-7B FOLIO (166/204 non-unanimous problems), per-trace constraint density is balanced between majority and minority answer groups ($6.0$ vs.\ $5.9$ constraints/trace) and MAX-SAT exclusion rates are comparable ($0.185$ vs.\ $0.172$); the extraction function $f$ produces constraints of equal quality regardless of answer group (\emph{extraction neutrality}).
The majority of unique formulas (${\sim}$74\%) are minority-exclusive, yet ${\sim}$26\% are shared across answer groups; under source-attribution, shared formulas vote for each side in proportion to trace count, providing zero discriminative signal (\emph{formula undirectedness}). Full semantic direction testing is precluded by variable isolation (56.4\% zero overlap).
Each constraint votes exclusively for its source trace's answer; volume amplification is ${\approx}1.0\times$. On 82.4\% of problems where the minority vote is correct, SICA still follows the majority (correction rate 17.6\%). SCB is therefore a transparency property: source-attribution scoring faithfully transmits the CoT trace distribution into verification without adding or removing information.

\paragraph{Answer-leakage controls.}
Two controls rule out answer contamination: (1)~BR is uniform across reasoning steps (Fisher $p = 0.71$); (2)~removing answer sentences before extraction yields no BR decrease (Wilcoxon $p \geq 0.12$) despite accuracy loss (McNemar $p < 0.013$). The bias reflects a distributional prior embedded throughout reasoning.
...
\paragraph{Semantic evaluation remark.}
\label{app:semantic-remark}
...
We recommend four lightweight diagnostics, computable from sampled traces alone, to predict whether symbolic self-verification will help or hurt before deployment.
...
\emph{Effective-K} ($K_\text{eff}$; Eq.~\ref{eq:effk}): compute Fleiss' $\kappa$ across $K$ traces, then $K_\text{eff} = K / [1 + (K-1)\kappa]$. Threshold: $K_\text{eff} \leq 1.30$ never improves in our experiments; aggregation collapses to majority voting. $K_\text{eff} > 1.8$ is a necessary (not sufficient) condition for gain.
